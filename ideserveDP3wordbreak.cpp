...
int check(vector<string> vs,string st){
     if(vs.size() == 0)
        return 0;
    ...
    for(int i=1;i<l;i++){
        if(vb[i]==-1 && find(vs.begin(),vs.end(),st.substr(0,i))!=vs.end())
            vb[i]=1;
        if(vb[i] == 1){
            if(i==l-1)
                return 1;
            for(int j=i+1;j<l;j++){
                if(vb[j]==-1 && find(vs.begin(),vs.end(),st.substr(i,j-i))!=vs.end())
                    vb[j]=1;
                else if(j==l-1 && vb[j]==1)
                    return 1;
            }
        } 
    }
